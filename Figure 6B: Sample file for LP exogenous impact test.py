...
DATA_PATH = Path(
    "/Users/MaJun/PycharmProjects/Python(Ajou University)/The Stability Trap Hypothesis/提交代码/Figure 6: COVID-19 event-driven impact/covid_lp_sample_from_panelFQ.csv"
)
...
def run_event_study_regression(df: pd.DataFrame) -> sm.regression.linear_model.RegressionResultsWrapper:
    ...
    # ===== Pre-trend joint test (matrix form): H0: beta_-4=beta_-3=beta_-2=beta_-1=0 =====
    pre_terms = ["Rigid_x_D_rel_-4", "Rigid_x_D_rel_-3", "Rigid_x_D_rel_-2", "Rigid_x_D_rel_-1"]
    names = list(results_es.params.index)

    missing_pre = [t for t in pre_terms if t not in names]
    if missing_pre:
        print("[WARNING] Pre-trend terms not found in params:", missing_pre)
    else:
        R = np.zeros((len(pre_terms), len(names)))
        for i, t in enumerate(pre_terms):
            R[i, names.index(t)] = 1.0

        # Use F-version of Wald test
        pre_test = results_es.wald_test(R, use_f=True)
        pval = float(pre_test.pvalue)
        fval = float(pre_test.statistic)
        df_num = getattr(pre_test, "df_num", len(pre_terms))
        df_denom = getattr(pre_test, "df_denom", None)

        print(f"[INFO] Pre-trend joint test (k=-4,-3,-2,-1): "
              f"F={fval:.3f}, p={pval:.4f}, df_num={df_num}, df_denom={df_denom}")
