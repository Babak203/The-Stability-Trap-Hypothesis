...
GAMMA_FILE = "/The Stability Trap Hypothesis/提交代码/Figure 4: Threshold grid search/threshold_point_ci.txt"
CLASSIFICATION = "/Users/MaJun/PycharmProjects/Python(Ajou University)/The Stability Trap Hypothesis/提交代码/Figure 2.3: Global fractal map + S-F scatter plots of 30 countries/classification_schemeA_v2_2023.csv"
USE_FIXED_GROUP = True
# 统一输出目录：图和表都写在这里
OUT_DIR = "/Users/MaJun/PycharmProjects/Python(Ajou University)/The Stability Trap Hypothesis/提交代码/Figure 5: LP dynamic effect"

# ========= 运行参数 =========
H_NEG: List[int] = [-3, -2]
H_POS: List[int] = [0, 1, 2, 3]
MIN_SIDE_N: int = 5
WINSOR_P: float = 0.01
COMMON_SAMPLE: bool = True     # True = 公共样本（更严），False = 按每个 h 取样
AUTO_FALLBACK: bool = True     # 公共样本若空则自动回退
GAMMA_FALLBACK: float = -1.061297  # 找不到文件时的兜底
...
def winsorize_inplace(df: pd.DataFrame, cols: List[str], p: float = 0.01) -> None:
    for c in cols:
        ...
        df[c] = x.clip(lo, hi)
def load_fixed_groups() -> pd.DataFrame:
    p = Path(CLASSIFICATION)
    if not p.exists():
        raise FileNotFoundError(f"找不到分类表：{p}")
    cls = pd.read_csv(p)

    # 低反馈：Rigid Trap + Fragile regimes
    low_set = set(cls.loc[cls["regimeA"].isin(["Rigid Trap", "Fragile regimes"]), "iso3c"].astype(str))

    out = cls[["iso3c", "regimeA"]].copy()
    out["country"] = out["iso3c"].astype(str)
    out["low_fb_fixed"] = out["iso3c"].astype(str).isin(low_set).astype(int)
    return out[["country", "regimeA", "low_fb_fixed"]]
...
def load_enriched() -> pd.DataFrame:
    ...
    if USE_FIXED_GROUP:
        grp = load_fixed_groups()
        df["country"] = df["country"].astype(str)
        df = df.merge(grp, on="country", how="left")
        # 如果有国家没匹配上，直接报错，避免悄悄变 NaN
        if df["low_fb_fixed"].isna().any():
            miss = df.loc[df["low_fb_fixed"].isna(), "country"].drop_duplicates().tolist()
            raise ValueError(f"这些国家在分类表里没找到：{miss}")
        df["low_fb_fixed"] = df["low_fb_fixed"].astype(int)
    # y: GDP 增长
    ...
    if "inv" not in df.columns:
        for c in ["WDI_Investment", "Investment", "inv", "NE.GDI.TOTL.ZS"]:
            if c in df.columns:
                df["inv"] = df[c]
                break
    assert "inv" in df.columns, "缺少 Investment 列（WDI_Investment / NE.GDI.TOTL.ZS 等）"

    # --- Trade ---
    if "trade" not in df.columns:
        for c in ["WDI_Trade", "Trade", "trade", "NE.TRD.GNFS.ZS"]:
            if c in df.columns:
                df["trade"] = df[c]
                break
    assert "trade" in df.columns, "缺少 Trade 列（WDI_Trade / NE.TRD.GNFS.ZS 等）"

    # --- Population growth ---
    if "popg" not in df.columns:
        for c in ["WDI_PopGrowth", "PopGrowth", "popg", "SP.POP.GROW"]:
            if c in df.columns:
                df["popg"] = df[c]
                break
    assert "popg" in df.columns, "缺少 PopGrowth 列（WDI_PopGrowth / SP.POP.GROW 等）"

    # 可选：winsorize 一下（跟你 g/pi 一样）
    winsorize_inplace(df, ["inv", "trade", "popg"], p=WINSOR_P)
    # 指标口径
    assert "S_PCA" in df.columns, "缺少 S_PCA"
    assert "F_PCA_main" in df.columns, "缺少 F_PCA_main"
    df = df.sort_values(["country", "year"]).copy()
    df["S1"] = df.groupby("country")["S_PCA"].shift(1)
    df["F1"] = df.groupby("country")["F_PCA_main"].shift(1)
    # 轻微稳健处理
...
def build_X(tmp: pd.DataFrame, gamma: float) -> pd.DataFrame:
    if USE_FIXED_GROUP:
        low = (tmp["low_fb_fixed"] == 1).astype(int)
    else:
        split_var = tmp["S1"] if THRESH_BY == "S" else tmp["F1"]
        low = (split_var < gamma).astype(int)
    ...
    return pd.DataFrame({
        ...
        "pi2":           tmp["pi2"].values,
        "inv": tmp["inv"].values,
        "trade": tmp["trade"].values,
        "popg": tmp["popg"].values,
    }, index=tmp.index)
...
def nested_masks(wide: pd.DataFrame, y_bank: dict, hs: List[int]) -> dict:
    ...
    for h in sorted(hs):
        ...
        need = ["y", "S1", "F1", "log_gdppc_lag", "pi", "pi2"]
        need += [c for c in ["inv", "trade", "popg"] if c in tmp.columns]
        valid = tmp[need].notna().all(axis=1)
...
def run_lp(df: pd.DataFrame, gamma: float, suffix: str = "") -> None:
    ...
    def fit_one(tmp: pd.DataFrame, h: int, diag: list):
        if USE_FIXED_GROUP:
            low_mask = (tmp["low_fb_fixed"] == 1)

            nL = tmp.loc[low_mask].index.get_level_values(0).nunique()
            nR = tmp.loc[~low_mask].index.get_level_values(0).nunique()

            # 现在 MIN_SIDE_N 才真正表示“每侧最少国家数”
        else:
            split_eff = tmp["S1"] if THRESH_BY == "S" else tmp["F1"]
            left_mask = (split_eff < gamma)

            # 统计国家数（cluster数），不是行数（country-year）
            nL = tmp.loc[left_mask].index.get_level_values(0).nunique()
            nR = tmp.loc[~left_mask].index.get_level_values(0).nunique()
        N = len(tmp)
        ...
        if common:
            ...
            if H_NEG:
                ...
                for h in H_NEG:
                    ...
                    base = ["y", "S1", "F1", "log_gdppc_lag", "pi", "pi2"]
                    extra = [c for c in ["inv", "trade", "popg"] if c in tmp.columns]
                    tmp = tmp.dropna(subset=base + extra).set_index(["country", "year"]).sort_index()
    ...
    fig3.savefig(diff_png, dpi=1000)
...
def main():
    ...
    df = load_enriched()
    print("[CHECK] cols contain low_fb_fixed?", "low_fb_fixed" in df.columns)
    if USE_FIXED_GROUP:
        print(df[["country", "low_fb_fixed"]].drop_duplicates().head())
