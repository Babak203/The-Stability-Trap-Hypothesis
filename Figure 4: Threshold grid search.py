...
class Config:
    ...
    # 阈值搜索参数
    min_per_side: int = 25
    coarse_q_low: float = 0.15
    coarse_q_high: float = 0.85
    ...
    # ===== Wild cluster bootstrap (for p-value of Δβ) =====
    use_wild_bootstrap: bool = True     # 计算 p_wild(Δβ)
    wild_B: int = 999                   # 999/1999
    wild_seed: int = 2026

    # 输出目录
    out_dir: str = os.path.join(os.path.dirname(__file__), "Figure 4: Threshold grid search")
    random_seed: int = 42
...
def clean_df(df, y_col, q_col, controls):
    ...
    before = len(df)
    df = df.dropna(subset=[y_col, q_col] + controls).copy()
    if CFG.jitter_std > 0:
...
def slope_diff_only(res, names) -> Tuple[float, float]:
    """只返回 diff 和 t（不算正态近似p），供 wild bootstrap 使用"""
    i1 = names.index("S_low")
    i2 = names.index("S_high")
    b1 = res.params[i1]
    b2 = res.params[i2]
...
def wild_cluster_pvalue(y2, X2, g2, res_fit, names, B=None, seed=None):
    """
    Restricted wild cluster bootstrap p-value for H0: Δβ = β_high - β_low = 0
    Uses: y* = yhat0 + e0 * w_cluster, where (yhat0,e0) come from restricted model.
    """
    import numpy as np
    import pandas as pd
    import statsmodels.api as sm
    ...
    if len(y2) != X2.shape[0] or len(y2) != len(g2):
        return np.nan
    ...
    i_low = names.index("S_low")
    i_high = names.index("S_high")

    # observed Δβ
    diff_obs = float(res_fit.params[i_high] - res_fit.params[i_low])
    if not np.isfinite(diff_obs):
        return np.nan

    # -------- restricted design matrix under H0: beta_low == beta_high --------
    # Keep const & controls, replace S_low & S_high with S_total = S_low + S_high
    S_total = (X2[:, i_low] + X2[:, i_high]).reshape(-1, 1)
    keep_idx = [j for j in range(X2.shape[1]) if j not in (i_low, i_high)]
    X_keep = X2[:, keep_idx]

    if "const" in names:
        j_const = names.index("const")
        const_pos = keep_idx.index(j_const)
        X0 = np.column_stack([X_keep[:, :const_pos+1], S_total, X_keep[:, const_pos+1:]])
    else:
        X0 = np.column_stack([S_total, X_keep])

    # restricted fit -> yhat0, e0
    res0 = sm.OLS(y2, X0).fit()
    yhat0 = np.asarray(res0.fittedvalues, dtype=float)
    e0 = np.asarray(res0.resid, dtype=float)

    # cluster indices
    idx_by_g = {ug: np.where(g2 == ug)[0] for ug in uniq}
    ...
    for b in range(int(B)):
        w = np.ones_like(e0, dtype=float)
        for ug, idx in idx_by_g.items():
            w[idx] = 1.0 if rng.integers(0, 2) == 1 else -1.0

        y_star = yhat0 + e0 * w

        try:
            res_star = sm.OLS(y_star, X2).fit()
            diffs[b] = float(res_star.params[i_high] - res_star.params[i_low])
        except Exception:
            continue
    ...
    count = int(np.sum(np.abs(diffs[ok]) >= abs(diff_obs)))
    B_eff = int(np.sum(ok))
    return float((count + 1) / (B_eff + 1))
...
def fit_at_gamma(df, gamma, y_col, q_col, controls, group_col):
    ...
    groups_all = df[group_col] if (group_col and group_col in df.columns) else None
    X_all, names, g_used = build_exog_slope(df, gamma, y_col, q_col, controls)

    res = ols_fit(y_all, X_all, groups_all)
    diff, t, p = slope_diff_tstat(res, names)

    # ===== 用 missing_row_idx 对齐到回归实际样本 =====
    n_all = len(y_all)
    miss = getattr(res.model.data, "missing_row_idx", None)

    mask = np.ones(n_all, dtype=bool)
    if miss is not None and len(miss) > 0:
        mask[np.asarray(miss, dtype=int)] = False
    ...
    if groups_all is not None:
        groups_used = pd.Series(groups_all).iloc[np.where(mask)[0]].reset_index(drop=True)

    # Wild bootstrap（Δβ 版，只跑一次）
    p_wild = np.nan
    if CFG.use_wild_bootstrap and (groups_used is not None):
        try:
            p_wild = wild_cluster_pvalue(
                y_used, X_used, groups_used, res, names,
                B=CFG.wild_B, seed=CFG.wild_seed
            )
        except Exception as e:
            print(f"[warn] wild bootstrap failed: {e}")
            p_wild = np.nan

    return {
        "gamma": g_used, "res": res, "names": names,
    ...
    for i in range(len(df)):
        ...
        try:
            ...
            rows.append({"i_drop": i, "gamma_used": ft["gamma"],
                         ...
                         "p_wild": float(ft.get("p_wild", np.nan))})
        except Exception as e:
...
def plot_grid(res_coarse, res_refine, gamma_point, out_png):
    ...
    fig, ax = plt.subplots(figsize=(7.2, 4.0), dpi=1000)
...
def plot_marginal_effect(res, names, gamma_point, Smin, Smax, out_png):
    ...
    # 分段：S ≤ γ* 与 S > γ*
    mask_low = xs <= gamma_point
    mask_high = xs > gamma_point

    # 两段边际效应曲线（legend 按你要求写）
    ax.plot(xs[mask_low], ys[mask_low], lw=1.0,
            label=r"$\partial g / \partial S$ (S <= gamma*)")
    ax.plot(xs[mask_high], ys[mask_high], lw=1.0,
            label=r"$\partial g / \partial S$ (S > gamma*)")

    # 95% CI（为了 legend 不重复，只给第一段加 label）
    if np.isfinite(ses).all():
        ax.fill_between(
            xs[mask_low],
            ys[mask_low] - 1.96 * ses[mask_low],
            ys[mask_low] + 1.96 * ses[mask_low],
            alpha=0.25,
            label="95% CI",
        )
        ax.fill_between(
            xs[mask_high],
            ys[mask_high] - 1.96 * ses[mask_high],
            ys[mask_high] + 1.96 * ses[mask_high],
            alpha=0.25,
            label=None,
        )
...
def run_all(df, y_col, q_col, group_col, controls, suffix=""):
    ...
    with open(txt, "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            f"t = {fit['t_diff']:.3f}, p(norm) = {fit['p_diff']:.4g}, "
            f"p(wild) = {fit.get('p_wild', np.nan):.4g}\n\n"
        )
    ...
    pd.DataFrame([{
        ...
        "p(Δβ)": fit["p_diff"],
        "p_wild(Δβ)": fit.get("p_wild", np.nan),
...
def table_m2_robust_pack(df, y_col, q_col, group_col,
    ...
    for label, opt in specs:
        ...
        try:
            if label == "A: Baseline":
                # ✅ 关键：用主图同一个 gamma_point，保证表图一致
                g_best = gamma_point
            else:
                # 其他稳健性规格：仍然允许自己找最优阈值（你也可以改成固定，见下一个方案）
...
def main():
    ...
    CFG.y_col = "WDI_GDP_Growth"
    CFG.q_col = "S_PCA"
    CFG.group_col = "countryiso3code"
    CFG.controls = ["WDI_Investment", "WDI_Inflation", "WDI_PopGrowth", "WDI_Trade"]
